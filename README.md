...
- **Lucky & Wild** (luckywld) - Better accuracy for player 2 and adjustments to default mappings. lr-mame has more analog controls than other MAME cores, and these unfortunately sometimes interfere with each other. As a result, we only set the things listed below as defaults. The rest should be mapped by the user. The L2 and R2 buttons can be used as analog inputs in lr-mame. R2 appears as analog axis Z, while L2 appears as analog axis RZ. To map as an analog button, you'd use `JOYCODE_1_ZAXIS_NEG_ABSOLUTE` (R1) or `JOYCODE_1_RZAXIS_NEG_ABSOLUTE` (L1) under the analog section for the particular input you want. To map as a digital button, you'd use `JOYCODE_1_ZAXIS_NEG_SWITCH` (R1) or `JOYCODE_1_RZAXIS_NEG_SWITCH` (L1) under the increasing portion of the digital controls for a particular input.
